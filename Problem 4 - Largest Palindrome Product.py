...
Description: Finding the palindrome using brute force which takes 0.9 seconds.
"""
result = []  # Add an empty list to append to later
for x in range(100, 1000):  # Checks all 3-digit numbers from 100 to 999
    for y in range(100, 1000):
        if str(x * y) == str(x * y)[::-1]:  # Checks for palindromes using an
            # extended slice of a string.
            result.append(x * y)  # Adds each palindrome product to result.
