"""
Project: Project Euler - Problem 74: Digit Factorial Chains
...
for number in range(1, 1000000):
    ...
    intermediate = set()
    while count <= 59:
        number_variable = sum(factorial(int(x)) for x in str(number_variable))
        intermediate.add(number_variable)
        count += 1
        if len(intermediate) == 59:
